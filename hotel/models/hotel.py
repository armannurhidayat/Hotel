...
class HotelFolioLine(models.Model):
    ...
    checkin_date = fields.Datetime(string='Check In', required=True, default=_get_checkin_date)
    checkout_date = fields.Datetime(string='Check Out', required=True, default=_get_checkout_date)
    is_reserved = fields.Boolean(string='Is Reserved', help='True when folio line created from \
                                 Reservation')
 ...
 -        '''
        ...
        if context.get('folio', False):
            if self.product_id and self.folio_id.partner_id:
                ...
                pr = self.product_id
                self.price_unit = tax_obj._fix_tax_included_price(pr.price, pr.taxes_id, self.tax_id)
            ...
            if self.folio_id.pricelist_id and self.folio_id.partner_id:
                ...
                val['price_unit'] = obj._fix_tax_included_price(p, pr.taxes_id, self.tax_id)
